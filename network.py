...
pan_pid = PID(p=0.01, d=0.007, i=0.03, imax=90)  # Adjust PID parameters
tilt_pid = PID(p=0.01, d=0.007, i=0.03, imax=90)  # Adjust PID parameters
...
def check_rectangle():
    ...
    for r in img.find_rects(threshold=10000):
       ...
       for g in range(4):
           rectanglePanList[g] = corners[g][0]
       for h in range(4):
           rectangleTiltList[h] = corners[h][1]
        ...
        if red_blob:
            ...
            if Signal == 9:
                for i in range(len(rectanglePanList)):
                    ...
                    target_y = rectanglePanList[i]
                    if not servo_pid_control(target_x, target_y, i):
                        break  # Exit the main loop if timeout occurs
                    print("loop {} is over".format(i))
                # back to orri
                target_x = rectanglePanList[0]
                target_y = rectanglePanList[0]
                if not servo_pid_control(target_x, target_y, 0):
                    break  # Exit the main loop if timeout occurs
                print("Mission accomplished")
